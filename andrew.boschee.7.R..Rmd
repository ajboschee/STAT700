...
CohenA <- data.frame(Year1, Year2, Mean1, Mean2, SD1, SD2)
...
Cohen_column <- cohen.wrapper(CohenA)

CohenA$Cohen <- Cohen_column
...
```{r}
CohenB <- CohenB[order(CohenB$Cohen),]

CohenB$ID <- seq.int(nrow(CohenB))
CohenB
CohenC <- CohenB[CohenB$ID %%2 == 0,]
CohenC
```
...
```{r}
CohenC <- CohenC[order(CohenC$Year2, CohenC$Year1),]
CohenC
```
...
```{r}
AppleData <- read.csv(file="AppleData.csv", header=TRUE)
AppleData <- AppleData[AppleData$tree ==3 | AppleData$tree == 7 | AppleData$tree == 10,]
summary(AppleData)
print(AppleData)
```
## Part b.
...
```{r}
print(apply(AppleWide, 2, mean))
print(tapply(AppleData$diam, AppleData$time, mean))
...
Difference <- NATR332.DAT$Y1-NATR332.DAT$Y2

NATR332.DAT$Difference <- Difference
...
```{r}
absDifference <- abs(NATR332.DAT$Difference)

Rank <- rank(absDifference)
NATR332.DAT$Rank <- Rank
...
```{r}
k <- length(NATR332.DAT$Difference)
SignedRank <- c()
for (i in 1:k){
 
  if(NATR332.DAT$Difference[i] < 0){
    SignedRank <- c(SignedRank, -1*Rank[i])
  }
  else{
    SignedRank <- c(SignedRank, Rank[i])
  }
}

NATR332.DAT$SignedRank <- SignedRank
...
for (i in 1:k){
  if(SignedRank[i] > 0){
    positiveRanks <- positiveRanks + SignedRank[i]
  }
  else{
    negativeRanks <- negativeRanks + abs(SignedRank[i])
  }
}
print(positiveRanks)
print(negativeRanks)
...
```{r}
  ...
  if(positiveRanks < negativeRanks){
   w <- positiveRanks
  }else{
    w <- negativeRanks
  }
...
$$z = (\mu_W - W)/\sigma_W$$
```{r}
N <- length(NATR332.DAT$Y1)
ExMeanMu <- (N*(N+1))/4
TwoN <- (N*2)+1
NTimesNPlusOne <- N*(N+1)


sigmaW <- sqrt(((NTimesNPlusOne*TwoN)/24))
...
AtmWtAg.long <- reshape(data=AtomicWeight,
                        idvar="ID",
                        v.names="Agwt",
                        varying=c("X1","X2"),
                        timevar="Instrument",
                        ...
                        direction="long")
AtmWtAg.long <- subset(AtmWtAg.long, select=-c(ID))
print(AtmWtAg.long)
...
```{r}
print(tapply(AtmWtAg.long$Agwt, AtmWtAg.long$Instrument, mean))
print(mean(AtomicWeight$X1))
print(mean(AtomicWeight$X2))
print(mean(AtomicWeight$X1) - mean(AtomicWeight$X2))
```
...
str(AtmWtAg.long$Instrument)
print(AtmWtAg.long$Instrument)
```
# Exercise 4
