...
```{r}
M_2 <- matrix(c(CaloriesPerRecipeMean), ncol=7, nrow=7)
M_1 <- matrix(c(CaloriesPerRecipeMean), ncol=7, nrow=7, byrow=TRUE)
s_1 <- matrix(c(CaloriesPerRecipeSD), ncol=7, nrow=7, byrow=TRUE)
s_2 <- matrix(c(CaloriesPerRecipeSD), ncol=7, nrow=7)
...
d.mat <- Cohen(M_2,M_1, s_1, s_2)
d_12 <- d.mat[idx.1936,idx.2006]
max <- max(d.mat)
print(max)
```
...
L_x1 <- likelihood_function(x1, mu, sigma)
L_x2 <- likelihood_function(x2, mu, sigma)
L_x3 <- likelihood_function(x3, mu, sigma)

l_1 <- L_x1[10]
l_2 <- L_x2[23]
l_3 <- likelihood_function(x3, mu, sigma)

plot(x3, l_3)

print(sum(x1)*tenth.increment)
print(sum(x2)*tenth.increment)
print(sum(x3)*tenth.increment)
...
```{r}
spr_mu <- ServingsPerRecipeMean[1]
spr_sigma <- 3*idx.1936



spr_sequence <- seq(from=spr_mu-spr_sigma, to=spr_mu+spr_sigma, by=tenth.increment)

print(sum(spr_sequence)*tenth.increment)

likelihood_function_spr <- 1/(2*pi*spr_sigma)*exp(1*(-1*((spr_sequence-spr_mu)^2))/(2*spr_sigma^2))


L <- likelihood_function_spr
plot(spr_sequence, L)
...
X <- matrix(c(Z,CaloriesPerServingMean),ncol=2, nrow = 7)
X_trans <- t(X)%*%X
X_inv <- solve(X_trans)
Y <- c(CaloriesPerRecipeMean)
y_trans <- t(X)%*%Y
...
```{r}
poisson_mu <- ServingsPerRecipeMean[1]
poisson_sigma <- ServingsPerRecipeSD[1]

poisson_Y <- seq(from=poisson_mu - poisson_sigma, to=poisson_mu + poisson_sigma, by=tenth.increment)

prob_mass <- exp(-(poisson_mu))*(1/(factorial(poisson_Y)))*(exp(1*(poisson_Y*log(poisson_mu))))
...
```{r}
likelihood_function_poisson <- likelihood_function(poisson_Y, poisson_mu, poisson_sigma)
L <- likelihood_function_poisson
...
```{r}
plot(y, f, type="b", col="red")
plot(y, L, type="s", col="blue")
```
...
```{r}
print(sum(f)*tenth.increment)
print(sum(L)*tenth.increment)
```
