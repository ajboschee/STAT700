...
SD2 <- c()
  ...
  for(i in 1:k-1)
    ...
    for(j in i+1:k){
      if(j-i != 1){ 
      Year1 <- c(Year1, Year[j-i])
      Mean1 <- c(Mean1, CaloriesPerRecipeMean[j-i])
      SD1 <- c(SD1, CaloriesPerRecipeSD[j-i])
      Year2 <- c(Year2, Year[i])
      Mean2 <- c(Mean2, CaloriesPerRecipeMean[i])
      SD2 <- c(SD2, CaloriesPerRecipeSD[i])
      }
      else{
        Year2 <- c(Year2,Year[i])
        Mean2 <- c(Mean2,CaloriesPerRecipeMean[i])
...
$$
\int _{x_0} ^{x_n} f(x) dx \approx \sum _{i=0} ^n w_i f(x_i)
$$
with $x_i = x_0 + h \times i$, $n$ is the number of $x_i$ in the series, step size $h$ and weight $w_i = h$.
...
```{r}
likelihood_function <- function(x, mu, sigma){
  sqrt_two_pi= sqrt(2*pi)
  return(1/(sqrt_two_pi)*exp(1*(-1*((x-mu)^2))/(2*sigma)))
}


h_val <- tenth.increment
mu <- 0
sigma <- 1
X <- seq(from=mu-sigma, to=mu+sigma, by=h_val)
length_val <- length(X)

for(i in 1:length_val){
  
  hvalue <- h_val
  X_0 <- seq(from=mu-sigma, to=mu+sigma, by=hvalue)
  L0val <- likelihood_function(X_0, mu, sigma)
  length_val <- length(L0val)
  for(j in 1:length_val){
  L_0<-sum(L0val)*hvalue}
}
...
for(i in 1:length_val){
  ...
  hvalue <- h_val/2
  X_1 <- seq(from= mu-sigma, to=mu+sigma, by=hvalue)
  L1val <- likelihood_function(X_1, mu, sigma)
  length_val<-length(L1val)
  for(j in 1:length_val){
  L_1<- sum(L1val)*hvalue}
}
  print(L_1)

  calcdelta <-function(Lseries,counternumber){
    counternumberdec<-counternumber-1
    deltaval <- Lseries[counternumber]-Lseries[counternumberdec]
    return(deltaval)}
...
for(i in length_val){
  hvalue <- (h_val/2)/2
  X_2 <- seq(from=mu-sigma, to=mu+sigma, by=hvalue)
  L2val <- likelihood_function(X_2, mu, sigma)
  length_val<-length(L2val)
  for(j in 1:length_val){
  L_2<-sum(L2val)*hvalue}
  print(L_2)
...
print(L_2)
print(hvalue)
print(L_2 - (pnorm(1, lower.tail = TRUE)-pnorm(-1, lower.tail = TRUE)))
...
```{r}
numeric_values <- c(24,56,34,76,34,45,67,78,45,65)


calculations <- function(values){
  k <- length(values)
  count <- 0
  count_sk <- 0
  count_kurt <- 0
  ...
  for(i in 1:k){
      ...
      x.bar<-mean(values)
      count <- count + (values[i]-x.bar)^2
      variance <- count/(length(values) -1)
      variance_skew <-count/(length(values))
      variance_kurt <-count/(length(values))
      st_dev_skew <-sqrt(variance_skew)
      st_dev <- sqrt(variance)
      count_sk <- count_sk + ((values[i]-x.bar)^3)
      sk_denom <- st_dev_skew ^3
      skewness <- (count_sk/sk_denom)/i
      count_kurt <- count_kurt + ((values[i]-x.bar)^4)
      st_dev_kurt <-sqrt(variance_kurt)
      kurt_denom <- st_dev_kurt^4
      kurtosis<- (count_kurt/kurt_denom)/i
    
  }
  return(list(Mean = x.bar,
              StandardDev = st_dev,
              Skewness = skewness,
              Kurtosis = kurtosis))
}  
...
NewJoyOfCooking <- na.omit(JoyOfCooking)
summary(NewJoyOfCooking)

calculations(NewJoyOfCooking$CaloriesperServing1936)
calculations(NewJoyOfCooking$ServingsperRecipe1936)
...
```{r}
gcd <- function(lower_num, higher_num)
{
  
  while(lower_num){
    temp = lower_num 
    lower_num = higher_num %% lower_num
    higher_num = temp
...
```{r}
rec.gcd <- function(lower_num, higher_num)
{
  if (higher_num %% lower_num == 0){
    return(lower_num)
  }
  else{
    return(rec.gcd(higher_num%%lower_num, lower_num))
  }
